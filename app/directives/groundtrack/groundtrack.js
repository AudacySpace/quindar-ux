                ...
                if(vehicles[i].dataStatus === true) {
                    ...
                    if(dServiceObjVal.sIcon === "green" && dServiceObjVal.gIcon === "green" &&
						...
						if (!est[i]){
							...
							//console.log((timeMission - timeDatabase)/1000)
							...
							// Get the current mission time
							var timeMission = dashboardService.getTime('UTC').today.getTime();
							var tInit = timeMission;	// Initial time	in milliseconds		
...
console.log(tmax);
	...
	// th: Angle of rotation [rad]
	// OUTPUTS
	// rNew: r expressed in a new frame rotated by th along z-axis
	function rot3(r, th)
	{
		var matA = rot3Mat(th);
		
		var rNew = new Array(3);
		
		for (iRow=0; iRow<3; iRow++)
			rNew[iRow] = matA[iRow][0]*r[0] + matA[iRow][1]*r[1] + matA[iRow][2]*r[2];
	...
	// OUTPUTS
	// rNew: Array [1 x 6] position, corrected velocity in the rotating frame
	function bke(r, omega)
	{
		...
		rNew[1] = r[1];
		rNew[2] = r[2];
		rNew[3] = r[3] + crossProd[0];
		rNew[4] = r[4] + crossProd[1];
		rNew[5] = r[5] + crossProd[2];
	...
	{
		var rNew = new Array(3);	// Initialize position array
		var vNew = new Array(3);	// Initialize velocity array
		
		// Position in ECEF
		rNew = rot3([r[0],r[1],r[2]], tGmst);	
	
		// Velocity expressed in ECEF
		vNew = rot3([r[3],r[4],r[5]], tGmst);

		// Use BKE to complete the velocity transformation
		// eAngV: Earth rotation rate
		var eAngV = 7.292115 * Math.pow(10,-5);	// rad/s
		var rFinal = bke([rNew[0],rNew[1],rNew[2],vNew[0],vNew[1],vNew[2]], [0.,0.,-eAngV]);
